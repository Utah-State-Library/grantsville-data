...
## 2024 Statistics for Grantsville City Library
...
gv_df %<>%
  mutate(
    Metric_pretty = case_when(
      ...
      Metric == "BKVOL" ~ "Print Materials",
      Metric == "AUDIO_PH" ~ "Audio Materials (Physical Copies)",
      Metric == "VIDEO_PH" ~ "Video Materials (Physical Copies)",
      Metric == "TOTPHYS" ~ "Total Physical Collection",
...
The collection has increased slightly over the past year, but circulation has increased at a faster pace. Increasing the collection size may help Grantsville City Library to meet the needs of its patrons.

```{r}
collection <- gv_df %>%
  filter(
    Metric %in%
      c("BKVOL", "AUDIO_PH", "VIDEO_PH", "TOTPHYS")
  ) %>%
  select(Metric_pretty, v_2023, v_2024, diff_pct)
...
circ <- gv_df %>%
  filter(
    Metric %in%
      c("PHYSCIR", "KIDCIRCL", "ELMATCIR", "TOTCIR")
  ) %>%
  select(Metric_pretty, v_2023, v_2024, diff_pct)
...
## Revenue - how does Grantsville compare?
...
ut_revenue <- utpls %>%
  ...
  group_by(FISCAL_YEAR, FSCSKEY) %>%
  mutate(percap = TOTINCM / POPU_LSA) %>%
  group_by(FISCAL_YEAR) %>%
  summarise(
    Library = "Utah Average",
    totincm_percap = round(mean(percap), 2)
  ) %>%
  distinct()
...
ggplot(revenue) +
  ...
  geom_line(
    aes(y = `Utah Average`, x = FISCAL_YEAR, colour = "Utah Average")
  ) +
  labs(
    title = "Revenue Per Capita",
    subtitle = "Compared to All Other Utah Libraries",
    caption = "The comparison line (in orange) represents all libraries except for Grantsville (in blue)",
    y = "Revenue per Capita",
    x = NULL,
    colour = NULL
  ) +
  scale_colour_manual(
    values = c("Grantsville City Library" = "blue", "Utah Average" = "orange")
  ) +
...
ggplot(revenue_50000) +
  ...
  geom_line(
    aes(
      y = `Utah Average (< 50,000)`,
      x = FISCAL_YEAR,
      colour = "Utah Average (< 50,000)"
    )
  ) +
  labs(
    title = "Revenue Per Capita",
    subtitle = "Compared to Libraries Serving < 50,000 People",
    caption = "The comparison line (in orange) represents all libraries except for Grantsville (in blue)",
    y = "Revenue per Capita",
    x = NULL,
    colour = NULL
  ...
  scale_colour_manual(
    values = c(
      "Grantsville City Library" = "blue",
      "Utah Average (< 50,000)" = "orange"
    )
  ) +
  theme_minimal(base_size = 16) +
  scale_x_continuous(breaks = 2015:2024) +
...
## Staffing - how does Grantsville compare?
...
ggplot(fte) +
  ...
  labs(
    title = "Staffing Per 1000 People",
    subtitle = "Compared to All Other Utah Libraries",
    caption = "The comparison line (in orange) represents all libraries except for Grantsville (in blue)",
...
FTE represents a 40 hour work week and does not necessarily align with the number of employees at the library. Some staff may be full time or part time. In 2024, Grantsville had 
`r n_staff` employees who split `r n_FTE` FTE.
